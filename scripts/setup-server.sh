...
if [ "$SKIP_ENV" = false ]; then
    ...
    echo -e "  ${BOLD}Database URL${NC} — connection string tới PostgreSQL database"
    echo -e "  Ví dụ: ${CYAN}postgresql://user:password@host:5432/dbname${NC}"
    ask "Database URL" DATABASE_URL ""

    if [ -z "$DATABASE_URL" ]; then
        error "DATABASE_URL là bắt buộc! Không thể bỏ trống."
    fi
...
CACHE_KEY_PREFIX=cp-system
CACHE_MEMORY_LRU_SIZE=5000
REDIS_URL=redis://redis:6379

# Judge server (Piston)
PISTON_API_URL=http://piston_api:2000/api/v2/execute
PISTON_CONTAINER=piston_api
PISTON_RUN_TIMEOUT=30000
PISTON_COMPILE_TIMEOUT=30000
PISTON_RUN_MEMORY_LIMIT=512000000
PISTON_COMPILE_MEMORY_LIMIT=512000000
PISTON_OUTPUT_MAX_SIZE=10485760
SUBMISSION_JUDGE_CONCURRENCY=2
...
info "Đợi API khởi động..."
for i in {1..30}; do
  if docker exec cp_api wget -q --spider http://localhost:3000/api 2>/dev/null; then
    success "API đã sẵn sàng"
    break
  fi
  if [ $i -eq 30 ]; then
    warn "API timeout — kiểm tra logs"
  fi
  sleep 2
done
...
echo -e "  ${CYAN}cd ${APP_DIR} && docker compose logs -f${NC}"
echo ""
