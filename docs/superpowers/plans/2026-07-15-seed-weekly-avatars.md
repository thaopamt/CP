...
- [x] **Step 1: Edit `apps/api/src/database/seeds/seed-shop.ts` to add `isActive?: boolean` to `ShopSeed` and add the three objects to `ITEMS`.**
- [x] **Step 2: Run the seed command to verify it runs successfully.**
...
- [x] **Step 1: Filter out items starting with `CHAR_WEEKLY_` in `ShopService.getCatalog`.**
- [x] **Step 2: Run build to verify types and compilation.**
