...
import { DP_BASIC_COURSES } from './dp-basic';
import { ProblemSpec } from './dp-basic/types';

const CLASS_CODE = 'DP-BASIC';
const COURSE_CODE_PREFIX = 'DP-BASIC-';
const ASSIGNMENT_SLUG_PREFIX = 'dpbasic-';
...
/** Languages students may submit in. The judge supports all of these. */
 ...
 * grading cases. Throws (failing the seed loudly) if a solver errors, so a
 * broken solver can never ship silently.
 */
function buildTestCases(p: ProblemSpec): { input: string; output: string; isHidden: boolean }[] {
  ...
  return p.inputs.map((input, i) => {
    ...
    try {
      ...
      throw new Error(
        `Solver for problem ${p.key} (${p.title}) threw on input ${JSON.stringify(input)}: ${err?.message ?? err}`
      );
    }
    if (typeof output !== 'string') {
      throw new Error(
        `Solver for problem ${p.key} (${p.title}) returned a non-string on input ${JSON.stringify(input)}.`
      );
    }
...
async function run() {
  console.log('🧠 Starting Seed Script for DP BASIC (Quy hoạch động cơ bản)...');
  ...
  let dpClass = await classRepo.findOneBy({ code: CLASS_CODE });
  if (!dpClass) {
    dpClass = new ClassEntity();
    dpClass.code = CLASS_CODE;
    dpClass.enrolledCount = 0;
  }
  dpClass.name = 'Quy hoạch động cơ bản';
  dpClass.description =
    'Giới thiệu các khái niệm nền tảng của quy hoạch động thông qua những bài tập ' +
    'cơ bản, dễ tiếp cận. Dành cho người mới học lập trình thi đấu, đã biết mảng, ' +
    'vòng lặp và đệ quy cơ bản.';
  dpClass.status = ClassStatus.ACTIVE;
  dpClass = await classRepo.save(dpClass);
  console.log(`✅ Class "Quy hoạch động cơ bản" ready (${dpClass.id}).`);

  // 3. Preload existing DP-BASIC courses & assignments for in-place UPSERT,
  ...
  // 4. Seed each course (chapter) and its problems.
  for (let c = 0; c < DP_BASIC_COURSES.length; c++) {
    const spec = DP_BASIC_COURSES[c];
    const courseNum = c + 1;
    ...
    let link = await classCourseRepo.findOne({ where: { classId: dpClass.id, courseId: savedCourse.id } });
    if (!link) link = new ClassCourse();
    link.classId = dpClass.id;
  ...
  const totalProblems = DP_BASIC_COURSES.reduce((s, c) => s + c.problems.length, 0);
  console.log(`\n🎉 DP BASIC seeded: ${DP_BASIC_COURSES.length} courses, ${totalProblems} problems.`);
  await AppDataSource.destroy();
}

run().catch((err) => {
  console.error('❌ Error during DP BASIC seed:', err);
  process.exit(1);
});
