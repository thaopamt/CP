...
export default function AssignmentEditPage() {
  ...
  const { data: implicitClassIds = [] } = useImplicitClasses(id);
  ...
  useEffect(() => {
    if (assignment) {
      ...
      if (assignment.codingConfig) {
        setTimeLimit(assignment.codingConfig.timeLimit || 1.0);
        setMemoryLimit(assignment.codingConfig.memoryLimit || 256);
  ...
  useEffect(() => {
    if (!id || hiddenZipFile || hiddenTestCount <= 0) {
      if (hiddenTestCount <= 0 && !hiddenZipFile) setHiddenTestcaseFiles([]);
      return;
    }

    let cancelled = false;
    assignmentsApi.getTestcaseManifest(id)
      .then((testcases) => {
        if (!cancelled) setHiddenTestcaseFiles(testcases);
      })
      .catch(() => {
        if (!cancelled) {
          setHiddenTestcaseFiles(
            Array.from({ length: hiddenTestCount }, (_, index) => ({
  ...
  }, [hiddenTestCount, hiddenZipFile, id]);
  ...
  const handlePublish = async () => {
    try {
      ...
      if (hiddenZipFile && id) {
        ...
        setHiddenTestcaseFiles(res.testcases);
        setHiddenZipFile(null);
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-md mb-xl">
          <div>
            <div className="flex items-center gap-sm mb-xs">
              ...
              <span className="material-symbols-outlined text-outline-variant text-sm">chevron_right</span>
              <span className="font-label-sm text-label-sm font-bold text-on-surface">Edit</span>
            </div>
            <h1 className="font-headline-lg text-headline-lg text-on-surface mb-xs">Edit Problem</h1>
            <p className="font-body-md text-body-md text-on-surface-variant">Update settings and content for the problem.</p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-lg">
          ...
          <div className="lg:col-span-8 space-y-lg">
            ...
            <section className="bg-surface-container-lowest rounded-xl border border-outline-variant p-lg shadow-sm">
              ...
              {testCases.length === 0 && (
                <div className="text-center py-md mb-md border border-dashed border-outline-variant rounded-lg bg-surface-container-lowest text-on-surface-variant text-sm">
                  No sample test cases have been added yet. Click the button below to add one.
                </div>
              )}
                ...
                <div>
                  ...
                  <div
                    ...
                    className={`border-2 border-dashed rounded-lg p-xl flex flex-col items-center justify-center text-center transition-colors cursor-pointer ${
                      dragOver ? 'border-primary bg-primary-container/20' : 'border-outline-variant bg-surface-container-low hover:bg-surface-container'
                    }`}
                  >
                    {uploading ? (
                      <>
                    ...
                    <div className="mt-md overflow-hidden rounded-lg border border-outline-variant bg-surface-container-lowest">
                      <div className="flex items-center justify-between gap-sm border-b border-outline-variant px-md py-sm">
                        ...
                        <span className="text-xs text-on-surface-variant">
                          {hiddenZipFile ? 'Preview from ZIP' : 'Stored on server'}
                        </span>
                      </div>
          ...
          <div className="lg:col-span-4 space-y-lg">
            ...
            <section className="bg-surface-container-lowest rounded-xl border border-outline-variant p-lg shadow-sm">
              ...
              <div>
                <label className="block font-label-sm text-label-sm text-on-surface-variant mb-xs mt-lg">Allowed Languages</label>
