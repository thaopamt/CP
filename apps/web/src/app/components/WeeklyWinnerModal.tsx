...
const AVATAR_MAPPING: Record<string, AvatarDetail> = {
...
export function WeeklyWinnerModal({ reward, onClose }: WeeklyWinnerModalProps) {
  ...
  const getRankTitle = () => {
    if (rank === 1) return t('gamif.student.leaderboard.weeklyWinner.titleChampion');
      ...
      >
        ...
        >
          <span className={`inline-block px-4 py-1.5 rounded-full text-xs font-black uppercase tracking-wider bg-white/5 border border-white/10 ${avatarDetails.textGlow}`}>
            🏆 {t('gamif.student.leaderboard.weeklyWinner.rankFormat', { rank })} • {getRankTitle()}
          </span>
        </motion.div>
        ...
        >
          {t('gamif.student.leaderboard.weeklyWinner.celebrationTitle')}
        </motion.h2>
        ...
        >
          {t('gamif.student.leaderboard.weeklyWinner.celebrationDesc', { xp: reward.weeklyXp.toLocaleString() })}
        </motion.p>
        ...
        >
          ...
          <div className="flex flex-col items-center justify-center p-2 rounded-xl bg-white/[0.02] border border-white/5 relative">
            ...
            <span className="text-[9px] text-slate-400 font-bold uppercase tracking-wider text-center truncate w-full">
              {t(`gamif.student.leaderboard.weeklyWinner.avatarName.${avatarCode}`)}
            </span>
            <span className="text-[8px] font-extrabold text-fuchsia-400 bg-fuchsia-500/10 border border-fuchsia-500/20 px-1 py-0.5 rounded mt-1 whitespace-nowrap flex items-center gap-0.5">
              ...
              {t('gamif.student.leaderboard.weeklyWinner.avatarDuration')}
            </span>
          </div>
        </motion.div>
